	...
	draw(x1,y1, x2,y2){
		for (let row = 0; row < this.rows; row++)
			for (let col = 0; col < this.cols; col++){
				...
				let scorecolor = map(this.getScore(col, row), 100, 0, 0, 255);
...
class Gradient{
	...
	update(x, y, size){
		let bottomcolor = color(255)
		let c = map(this.maxv,0,100, 0, 255)
		let topcolor = color(this.selection[0]*c, this.selection[1]*c,this.selection[2]*c)

		noFill();

		this.draw(x, y, size.width, size.height, bottomcolor, topcolor) 
	}
}
